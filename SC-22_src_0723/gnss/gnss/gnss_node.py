...
class GNSSNode(Node):
    def __init__(self):
        ...
        """
        self.publisher_gnss_init = self.create_publisher(NavSatFix, '/gps/fix/init', 10)
        self.publisher_gnss = self.create_publisher(NavSatFix, '/gps/fix', 10)
        self.init_latitude,self.init_longitude = self.getData()
    ...
    def gnss_callback(self):
        ...
        msg_gnss_init = NavSatFix()
        msg_gnss_init.header.stamp.sec = self.get_clock().now().to_msg().sec
        msg_gnss_init.header.stamp.nanosec = self.get_clock().now().to_msg().nanosec
        msg_gnss_init.header.frame_id = "map"
        msg_gnss_init.latitude = self.init_latitude
        msg_gnss_init.longitude = self.init_longitude
        self.publisher_gnss_init.publish(msg_gnss_init) 
        
        msg_gnss = NavSatFix()

        msg_gnss.header.stamp.sec = self.get_clock().now().to_msg().sec
        msg_gnss.header.stamp.nanosec = self.get_clock().now().to_msg().nanosec
        msg_gnss.header.frame_id = "base_footprint"
        
        latitude,longitude = self.getData()
        msg_gnss.latitude = latitude
        msg_gnss.longitude = longitude
                        ...
                        if 10 <= x <= 126:
                            ...
                            if stat:
                                ...
                                if (tm_now - tm_last) >= 10:
                                    ...
                                    return my_gnss.latitude[0],my_gnss.longitude[0]
